...
final class WasserStore: ObservableObject {
    ...
    private let localKey = "wasser_eintraege_v1"
    ...
    func fetch() async {
        do {
            ...
            var byID: [UUID: (WasserEintrag, CKRecord.ID)] = [:]
            for (recordID, result) in results {
                if case .success(let record) = result, let entry = WasserEintrag(record: record) {
