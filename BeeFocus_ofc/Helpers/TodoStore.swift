...
class TodoStore: ObservableObject {
    ...
    init() {
        ...
        CloudKitManager.shared.fetchDailyStats { [weak self] cloudDaily in
            self?.mergeDailyStatsFromCloud(cloudDaily)
        }
        CloudKitManager.shared.fetchFocusStats { [weak self] cloudFocus in
            self?.mergeDailyFocusMinutesFromCloud(cloudFocus)
        }
    ...
    // Cloud merge helpers
    func mergeDailyStatsFromCloud(_ cloud: [Date: Int]) {
        var changed = false
    ...
    func mergeDailyFocusMinutesFromCloud(_ cloud: [Date: Int]) {
        var changed = false
